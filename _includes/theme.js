...
let setPlotlyTheme = (theme) => {
  document.querySelectorAll(".js-plotly-plot").forEach((elem) => {
    ...
    // Some custom Plotly charts (e.g., dynamically rendered widgets) do not have this sibling block.
    const codeBlock = elem.previousSibling;
    if (!codeBlock || !codeBlock.childNodes || !codeBlock.childNodes[0] || !codeBlock.childNodes[0].innerHTML) {
      return;
    }

    let jsonData;
    try {
      jsonData = JSON.parse(codeBlock.childNodes[0].innerHTML);
    } catch (_e) {
      return;
    }
...
let initTheme = () => {
  ...
  let bindThemeToggle = () => {
    ...
    if (mode_toggle.dataset.themeBound === "1") return;

    mode_toggle.addEventListener("click", function (event) {
      event.preventDefault();
      toggleThemeSetting();
    });
    mode_toggle.dataset.themeBound = "1";
  };
